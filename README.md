...
There are other esoteric programming languages that use music as code. One notable example is **Velato**, which uses MIDI files as source code. In Velato, the pitch of the first note in a measure determines the command, and the intervals between notes determine parameters. Like MusicCoder, it aims to produce code that sounds like actual music, although the constraints of the language often result in jazz-like or atonal compositions. Other examples include **Fugue** and **Choon**, which also explore the intersection of algorithmic logic and musical notation.
